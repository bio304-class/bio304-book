...
* Standard bivariate and multiple regression assumes that the predictor variables ( ($X_1, X_2, \ldots$) are observed without error.
...
* If the explanatory variables ($X_1, X_2,\ldots,X_m$) are highly correlated, then the regression solution can be "unstable" -- a small change in the data could lead to a very large change in the regression model.
...
Let's create also do some quick data summaries for our variables:
...
And a correlation matrix to summarize the bivariate association between the variables:

```{r}
cor(mtcars.subset)
```

Let's also use the `GGally::ggpairs` function to create visualization of the bivariate relationships:
...
Since we're building a model that involves three variables, it makes sense to look at at 3D plotl. ggplot has no built in facilities for 3D scatter plots so we'll use a package called  [`plot3D`](http://www.rforscience.com/rpackages/visualisation/plot3d/). `plot3D` follows the plotting conventions of the base R-graphics capabilities, so we can't build up figures in layers.  Instead we pass all the formatting arguments to a single function call.  

To create a 3D scatter plot we can use the `plot3D::points3D` function. The argument `pch` sets the type of plotting character to use in the plot (for agraphical key of the available plotting characters see [this link](https://www.statmethods.net/advgraphs/parameters.html)).
...
Once you've installed `rgl` try the following code.  To make this work with knitr, make your codeblock start with this header: `{r, webgl=TRUE, results = "asis"}`
