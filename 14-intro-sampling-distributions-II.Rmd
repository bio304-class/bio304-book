...
ggplot(height.data, aes(x = height)) +
  # note the use of y = ..density.. to generate a 
  # density (relative frequency) histogram
...
sample.a %>%
  ggplot(aes(x = height)) + 
  geom_histogram(data=height.data, aes(x = height, y = ..density..),
                 alpha=0.25, bins=50) + 
  geom_histogram(aes(y = ..density..), 
                 fill = 'steelblue', alpha=0.75, bins=9) + 
...
ggplot(sample.b, aes(x = height)) + 
  geom_histogram(data=height.data, 
                 aes(x = height, y = ..density..), alpha=0.25, bins=50) + 
  geom_histogram(aes(y = ..density..), 
                 fill = 'steelblue', alpha=0.75, bins=9) +
  geom_vline(xintercept = true.values$mean.height, linetype = "solid") + 
...
First we're going to write a function called `rsample.stats` that carries out the following steps:
