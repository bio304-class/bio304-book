...
* *Complement* -- the complement of an event is all the possible outcomes of a random trial that are *not* the event.
...
* Let $A^c$ be the complement of the event $A$. Then $P(A^c) = 1 - P(A)$
...
* Cards: In randomly shuffled standard (French) 52-card deck, the probability of drawing a heart is 13/54 = 0.25; the probability of getting an ace is 4/52 = ~0.077; the probability of drawing the ace of hearts is 1/52 = ~0.0192 
...
```{r, fig.cap = "Figure 2. Distribution of total SAT scores for 2017 high school graduates. Assuming a normal distribution, $N(1060, 195)$, based on data  reported in the [2017 SAT annual report](https://reports.collegeboard.org/pdf/2017-total-group-sat-suite-assessments-annual-report.pdf). The probability that a randomly cohse student got a score better than 1255 is represented by the shaded area; P(Score > 1255) = 0.1587.", fig.align='center', echo = FALSE}
